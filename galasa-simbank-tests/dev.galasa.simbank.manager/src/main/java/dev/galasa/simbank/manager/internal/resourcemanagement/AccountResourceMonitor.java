...
public class AccountResourceMonitor implements Runnable {
    ...
    private final Pattern                    serverPattern = Pattern
            .compile("^run\\.(\\w+)\\.instance\\.(\\w+)\\.account\\.(\\w+)$");

    public AccountResourceMonitor(IFramework framework, IResourceManagement resourceManagement,
    ...
    private void discardAccount(String instance, String accountNumber, String runName)
            throws DynamicStatusStoreException {
        logger.info("Discarding SimBank Account " + accountNumber + " on instance " + instance + " as run " + runName
                + " has gone");
    ...
    private void deleteAccount(String instance, String accountNumber) {
        ...
        try {
            String jdbcUri = AbstractManager
                    .nulled(dss.get("instance." + instance + ".account." + accountNumber + ".database"));
        ...
        try {
            ...
            for (String key : serverRuns.keySet()) {
                ...
                if (matcher.find()) {
                    ...
                    // try {
                    // OpenstackServerImpl.deleteServerByName(serverName, runName, dss,
                    // this.openstackHttpClient);
                    // } catch(Exception e) {
                    // logger.error("Failed to discard OpenStack server " + serverName + " for run "
                    // + runName);
                    // }
                }
            }
